...
newstopwords=stopwords.words("English") + ['yuhao','the','is','it','may','shutao'] # add some stopwords by domain knowledge
...
X_train, X_test, y_train, y_test = train_test_split(subnews.Text, subnews.Class, test_size=0.3, random_state=12)

#Create DTM (document-term matrix) by using word occurence
...
#Set use_idf=False means use tf table only, not tf-idf.
...
# # Building Model by using Naïve Bayes
from sklearn.naive_bayes import MultinomialNB
...
print('Result of Naïve Bayes:')
print(metrics.confusion_matrix(y_test, predicted)) # Confusion Matrix
print(np.mean(predicted == y_test)) # Correct Rate

# # Building Model by using Decision Tree Model
...
print('Result of Support Vector Machine:')
print(metrics.confusion_matrix(y_test, predicted))
print(np.mean(predicted == y_test) )
...
## Confusion Matrix Output Optimize
import itertools
import matplotlib.pyplot as plt
